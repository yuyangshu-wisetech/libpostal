...
class NeighborhoodReverseGeocoder(RTreePolygonIndex):
    ...
    def create_from_osm_and_wof(cls, filename, wof_dir, country_rtree_dir, osm_rtree_dir, osm_neighborhood_borders_file, output_dir):
        ...
        wof = WhosOnFirstNeighborhoodsReverseGeocoder.create_neighborhoods_index(wof_dir)
...
class WhosOnFirstNeighborhoodsReverseGeocoder(GeohashPolygonIndex):
    ...
    @staticmethod
    def is_valid_neighbourhood(geojson):
        return not geojson["properties"].get(SUPERSEDED) and geojson["properties"].get(LEVEL) in NEIGHBOURHOOD_TYPES and geojson["geometry"]["type"] != "Point"

    @classmethod
    def create_neighborhoods_index(cls, wof_dir, output_dir, index_filename=None, polys_filename=DEFAULT_POLYS_FILENAME):
        filtered_files = []
        for root, dirnames, filenames in os.walk(wof_dir):
            for fname in fnmatch.filter(filenames, "*.geojson"):
                with open(os.path.join(root, fname)) as f:
                    geojson = json.load(f)
                    if is_valid_neighbourhood(geojson):
                        filtered_files.append(os.path.join(root, fname))
        return cls.create_from_geojson_files(filtered_files, output_dir, index_filename=index_filename, polys_filename=polys_filename)
